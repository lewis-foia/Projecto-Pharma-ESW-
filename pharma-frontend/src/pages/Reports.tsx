...
export default function Reports() {
  ...
  const generate = async () => {
    ...
    setError(null);
    try {
      const res = await api.getReport(type, period);
      setData(res);
    } catch (err: any) {
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 flex items-center gap-3">
          <BarChart3 className="w-8 h-8 text-purple-600" /> Relatórios
        </h1>
        <p className="text-sm text-gray-500 mt-1">Gerar relatórios do sistema</p>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm mb-6 flex flex-wrap gap-4 items-end">
        ...
        <button onClick={generate} disabled={loading} className="bg-purple-600 hover:bg-purple-700 disabled:bg-purple-300 text-white px-6 py-2 rounded-lg transition">
